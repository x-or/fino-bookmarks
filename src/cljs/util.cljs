...
(defn ^:export get-data-fail-safe [url ok-func err-func]
  (.send goog.net.XhrIo
         url
         #(let [self (.-target %)] 
           (if (= 200 (.getLastErrorCode self))
              (ok-func (reader/read-string (.getResponseText self)))
              (err-func self)))
         "GET"))
...
(defn ^:export post-data-fail-safe [url ok-func err-func]
  (.send goog.net.XhrIo
         url
         #(let [self (.-target %)] 
           (if (= 0 (.getLastErrorCode self))
              (ok-func (reader/read-string (.getResponseText self)))
              (err-func self)))
         "POST"))
